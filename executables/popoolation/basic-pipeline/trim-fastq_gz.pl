...
#--input /Volumes/Volume_3/analysis/clipsignal/raw/s100000.fastq --output /Users/robertkofler/tmp/filter.test.fastq
    ...
    if(-e $input2)
    {
        ...
        MainProc::processPE($input1,$input2,$output,$encoder,$trimmingalgorithm,$trimQuality,$qualThreshold,$processStep,$minLength,$discardRemainingNs,$no5ptrim,$verbose,$nozip);
    }
    else
    {
        print  "Did not find an existing file for the second read; Switching to single-read mode\n";
        MainProc::processSE($input1,$output,$encoder,$trimmingalgorithm, $trimQuality,$qualThreshold,$processStep,$minLength,$discardRemainingNs,$no5ptrim,$verbose,$nozip);
    ...
    
{
	    ...
	    if ($nozip) 
	      {
	       open $ofh, ">", $output or die "Could not open output file $!";
	      }
	    else
	      {
		$output=$output . ".gz";
		$ofh = new IO::Compress::Gzip $output or die "Could not open gzipped output file $output $!"; 
#		open $ofh, "| gzip > $output" or die "Could not open gzipped output file $output $!"; 
	      }
	    ...
	    if ($nozip) 
	      {
		open $ofh1, ">", $output1 or die "Could not open output file";
		open $ofh2, ">", $output2 or die "Could not open output file";
		open $ofhse, ">", $outputse or die "Could not open output file";
	      }
	    else
	      {
		$output1 = $output1 . ".gz";
		$output2 = $output2 . ".gz";
		$outputse = $outputse . ".gz";		
		# $ofh1 = new IO::Compress::Gzip $output1 or die "Could not open gzipped output file $output1 $!"; 
		# $ofh2 = new IO::Compress::Gzip $output2 or die "Could not open gzipped output file $output2 $!"; 
		# $ofhse = new IO::Compress::Gzip $outputse  or die "Could not open gzipped output file $outputse $!"; 
		open $ofh1, "| gzip > $output1" or die "Could not open gzipped output file $output1 $!"; 
		open $ofh2, "| gzip > $output2" or die "Could not open gzipped output file $output2 $!"; 
		open $ofhse, "| gzip > $outputse" or die "Could not open gzipped output file $outputse $!"; 
    ...
    
{
    ...
    sub new
    {
        ...
        my $file=shift;
	my $ofh = undef;	
	#my $ofh = new IO::Uncompress::AnyUncompress $file or die "Could not open file gzipped file $file - do you have zlib $!";
	if ($file=~/\.gz$/i) {
	  #my $ofh = new IO::Uncompress::Gunzip $file or die "Could not open file gzipped file $file - do you have zlib $!";
	  open $ofh, "gzip -dc $file |" or die "Could not open file gzipped file $file - do you have zlib $!";
	}
	else {
	...
	# aditional checks
	#die  "first header length must be greater 2, line: $firstheader" unless length($firstheader) < 2;
	#die  "quality and sequence length differ, line: $firstheader" unless length($quality) == length($nucleotide);
...
The input file, or the input file of the first read, in fastq format.
Input files can be in gzipped form, but have to end in a ".gz" suffix.
Mandatory parameter
...
The output file. Will be in fastq. By default the output files are gzipped. Mandatory parameter
...
togle switch; Disable default gzipped output of trimmed FastQ files. default=off
