...
if (defined($opt_d)) {
	...
	if (0) {
	    ...
	    $FP = new IO::File("| $ENV{"DISTMAP_PIERRE"}/gmap-2015-12-31/bin/get-genome -D $opt_D -d $opt_d > get-genome.out");
	} else {
	    $FP = new IO::File("| $ENV{"DISTMAP_PIERRE"}/gmap-2015-12-31/bin/get-genome -d $opt_d > get-genome.out");
	}
