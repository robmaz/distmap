...
int NV_MPI_Send(void* buf, int count, NV_Datatype datatype, int task, int tag)
{
    ...
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    mpitracef("R%d %lx %d %d %x", rank, (long unsigned int)buf, count, datatype, dtConvert[datatype]);
#endif
...
int NV_MPI_Recv(void * msg, int msg_len,  NV_Datatype datatype)
{
    ...
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    mpitracef("R%d L:%d", rank, msg_len);
#endif
...
#endif
    MPI_Recv(msg, msg_len,  dtConvert[datatype], MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, (MPI_Status *)status);
}
