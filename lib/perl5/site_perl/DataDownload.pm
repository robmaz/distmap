...
sub paired_end_data {
    ...
    my $file_count = $self->get_file_list("$args_dict->{'output_directory'}/$args_dict->{'local_home'}/$args_dict->{'read_folder'}");
	...
	foreach my $mapper_name (@{$args_dict->{"mapper"}}) {
		...
		my $local_output_dir = "$args_dict->{'output_directory'}/$args_dict->{'local_home'}/$args_dict->{'read_folder'}"."_mapping_".lc($mapper_name);
		Utility::deletedir($local_output_dir);


		my $output_folder = "$output/$args_dict->{'read_folder'}"."_mapping_".lc($mapper_name);

		my $download_command = "$hdfs_exe dfs -D dfs.blocksize=$args_dict->{'block_size'} -copyToLocal $output_folder $args_dict->{'output_directory'}/$args_dict->{'local_home'}/";
...
sub single_end_data {
    ...
    if ($file_count>0 or $hdfs_file_count>0) {
        ...
        #### Deleting the download folder if already exists
        #Utility::deletedir("$args_dict->{'output_directory'}/$args_dict->{'local_home'}/$args_dict->{'read_folder'}"."_mapping");
        #
        #my $download_command = "$hdfs_exe dfs -copyToLocal $output_folder $args_dict->{'output_directory'}/$args_dict->{'local_home'}/";
        #
        #print STDERR "Data download from hdfs file system started ", localtime(), "\n";
        #system($download_command) == 0 || warn "Error in uploading the $args_dict->{'job_arch'}\n";
        #print STDERR "Data download from hdfs file system finished ", localtime(), "\n";
