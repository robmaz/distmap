...
sub download_merge_trimmed_reads {
	...
	if ((system("$hdfs_exe dfs -test -d $output_folder_trim")==0)) {
		...
		Utility::deletedir("$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_folder'}"."_trimming");


		my $download_command = "$hdfs_exe dfs -copyToLocal $output_folder_trim $args_dict->{'output_directory'}/$args_dict->{'random_id'}/";
...
sub merge_trimmed_reads {
    ...
    my $temp_output_folder = "$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_folder'}"."_trimming";
	...
	my $output_file = $args_dict->{"output_directory"}."/".$args_dict->{'random_id'};
