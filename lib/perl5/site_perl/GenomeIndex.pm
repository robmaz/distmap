...
sub copy_exec {
	my ($self,$args_dict) = @_;
	#my $bin_dir_path = "$args_dict->{'output_directory'}/$args_dict->{'local_home'}/$args_dict->{'bin_dir'}";
...
sub bismark_index {
	...
	unless (-e $bowtie_path and -x $bowtie_path) {
		...
		$bowtie_path = "$args_dict->{'output_directory'}/$args_dict->{'local_home'}/$args_dict->{'bin_dir'}/bowtie";

		unless (-e $bowtie_path and -x $bowtie_path) {
			$bowtie_path = "$args_dict->{'output_directory'}/$args_dict->{'local_home'}/$args_dict->{'bin_dir'}/bowtie2";
		}
		unless (-e $bowtie_path and -x $bowtie_path) {
...
sub bowtie_index {
	...
	unless (-e $bowtie_build and -x $bowtie_build) {
		$bowtie_build = "$args_dict->{'output_directory'}/$args_dict->{'local_home'}/$args_dict->{'bin_dir'}/bowtie-build";

		unless (-e $bowtie_build and -x $bowtie_build) {
			$bowtie_build = "$args_dict->{'output_directory'}/$args_dict->{'local_home'}/$args_dict->{'bin_dir'}/bowtie-build2";
		}
		unless (-e $bowtie_build and -x $bowtie_build) {
			print "\n\tERROR: bowtie-build or bowtie2-build not found  in the directory \"$path\"\n\n";
			#exit(1);
