...
sub paired_end_trimming {
	...
	my $shell_script = $args_dict->{"output_directory"}."/$args_dict->{'random_id'}/trim_pe_hadoop.sh";
...
sub single_end_trimming {
	...
	my $shell_script = $args_dict->{"output_directory"}."/$args_dict->{'random_id'}/trim_se_hadoop.sh";
	$args_dict->{"shell_script"} = $shell_script;
