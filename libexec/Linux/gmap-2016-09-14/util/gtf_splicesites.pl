...
if (defined($opt_d)) {
	...
	if (0) {
	    ...
	    $FP = new IO::File("| $ENV{DISTMAP_HOME}/executables/gmap-2016-09-14/bin/get-genome -D $opt_D -d $opt_d > get-genome.out");
	} else {
	    $FP = new IO::File("| $ENV{DISTMAP_HOME}/executables/gmap-2016-09-14/bin/get-genome -d $opt_d > get-genome.out");
	}
