...
sub clean_hdfs_files {
    ...
    #print "$input\t$output\n";
    system("$hdfs_exe dfs -rm -r $input") == 0
      || warn
"Error in deleting the $input folder on hdfs file system. $input does not exists\n";
    system("$hdfs_exe dfs -rm -r $output") == 0
      || warn
"Error in deleting the $output folder on hdfs file system. $input does not exists\n";
