...
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_folder'}"
        ...
        my $download_command =
"$hdfs_exe dfs -copyToLocal $output_folder_trim $args_dict->{'output_directory'}/$args_dict->{'random_id'}/";
...
sub getofhcreater {
    ...
    if ($nozip) {
        ...
        $outfile = $outfile . ".gz";
        $ofh     = new IO::Compress::Gzip $outfile
          or die "Could not open gzipped output file $outfile $!";
    }
    return $ofh;
}
