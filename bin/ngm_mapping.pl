...
#Usage:  ngm [-c <path>] {-q <reads> [-p] | -1 <mates 1> -2 <mates 2>} -r <reference> -o <output> [parameter]
    my $cmd1 =
"$mapper_path  -p -1 $read1_fastq -2 $read2_fastq -r $ref  --bam --sensitive -t 2 --local $mapper_args";
    print STDERR "cmd1: $cmd1\n";
    Utility::runCommand( $cmd1, "ngm of $read1_fastq and $read2_fastq" ) == 0
      || die "Error bwa aln of $read1_fastq and $read2_fastq";
...
#Utility::runCommand("$hdfs dfs -put $bam_output $output_dir/ >&2", "hdfsp dfs -put") == 0 || die "hdfs dfs -put command failed";
    ...
    my $cmd1 =
"$mapper_path -q $read1_fastq -r $ref --bam --local $mapper_args -o $bam_output";

    print STDERR "cmd1: $cmd1\n";
    Utility::runCommand( $cmd1, "ngm of $read1_fastq" ) == 0
