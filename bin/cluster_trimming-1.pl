...
if ($read_type1 =~ /(pe|pair)/i) {
	...
	Utility::runCommand($cmd1, "perl trim-fastq.pl of $read1_fastq and $read2_fastq") == 0 || die "Error perl trim-fastq.pl of $read1_fastq and $read2_fastq";
	...
	my $cmd1 = "perl trim-fastq.pl $trim_args --input1 $read1_fastq --output1 $read1_fastq_out > $std_output";
	print STDERR "cmd1: $cmd1\n";
	
	Utility::runCommand($cmd1, "perl trim-fastq.pl of $read1_fastq") == 0 || die "Error bwa aln of perl trim-fastq.pl of $read1_fastq";
