...
if ( system("$hdfs dfs -test -e $output_dir/$merged_output") == 0 ) {
    exit();
...
"perl trim-fastq.pl $trim_args --disable-zipped-output --input1 $read1_fastq --input2 $read2_fastq --output1 $read1_fastq_out --output2 $read2_fastq_out --outputse $read_se_output > $std_output";
    ...
    open my $fh1, $read1_fastq_out or die "could not open $_";
    open my $fh2, $read2_fastq_out or die "could not open $_";
    ...
    my $cmd1 =
"perl trim-fastq.pl $trim_args --input1 $read1_fastq --output1 $read1_fastq_out > $std_output";
    print STDERR "cmd1: $cmd1\n";

    Utility::runCommand( $cmd1, "perl trim-fastq.pl of $read1_fastq" ) == 0
      || die "Error bwa aln of perl trim-fastq.pl of $read1_fastq";

    open( my $ofh, ">" . $merged_output ) or die "could not open $_";
    open my $fh, $read1_fastq_out or die "could not open $_";
