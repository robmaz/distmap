...
sub compare_fastq_bam_reads {
    ...
    if ( $args_dict->{"trimming_flag"} ) {
        ...
        my $download_command =
          "$hadoop_exe dfs -getmerge $output_folder_trim $temp_file";
...
#time /Volumes/cluster/DistMap_v1.2.1/executables/samtools-0.1.19/samtools idxstats /Volumes/cluster/Ray/BGI_143b/DistMap_output_Paired_end_reads.bam | awk '{i+=$3+$4} END {print i}'
    ...
    my $command2 =
"$samtools idxstats $final_output_file | awk '{i+=\$3+\$4} END {print i}' > $mapped_file_std";
...
"\n\tERROR: missing_reads reads are missing from BAM file\n\tERROR: Fastq file read count: $total_reads and BAM file read count: $mapped_reads\n";
        print STDERR
"\tERROR: Run command with --only-hdfs-download --only-merge two flags\n\n";
...
#$command = "perl $merge_fastq_script --output $output_file --hdfs-dir $args_dict->{'output_folder'} --hdfs $hadoop_exe --disable-zipped-output &";
    ...
    my $command =
"perl $merge_fastq_script --output $output_file --hdfs-dir $args_dict->{'output_folder'} --hdfs $hadoop_exe --disable-zipped-output &";
...
sub paired_end_data {
 ...
 #$args_dict->{"read_output_folder"} = "$args_dict->{'read_folder'}"."_mapping";
    ...
    foreach my $mapper_name ( @{ $args_dict->{"mapper"} } ) {
        ...
        $args_dict->{"read_output_folder"} =
          "$args_dict->{'read_folder'}" . "_mapping_" . lc($mapper_name);
        $args_dict->{"final_output_file"} =
          "DistMap_output_Paired_end_reads_" . lc($mapper_name);

        my $file_count = 0;
        my $to_read_dir =
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_output_folder'}";
        if ( -d $to_read_dir ) {
...
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_output_folder'}"
        ...
        if ( $file_count > 0 or $hdfs_file_count > 0 ) {
            ...
            my $output_dir =
              "$args_dict->{'output_directory'}/$args_dict->{'random_id'}";
            my $bam_output_file =
              "$args_dict->{'output_directory'}/$output_file";
        }
 ...
 #$args_dict->{"read_output_folder"} = "$args_dict->{'read_folder'}"."_mapping";
    ...
    foreach my $mapper_name ( @{ $args_dict->{"mapper"} } ) {
        ...
        $args_dict->{"read_output_folder"} =
          "$args_dict->{'read_folder'}" . "_mapping_" . lc($mapper_name);
        $args_dict->{"final_output_file"} =
          "DistMap_output_Single_end_reads_" . lc($mapper_name);
...
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_output_folder'}";
    ...
    my $picard_temp =
      ...
      . $mapper_name
      . "_picard_tmp";
    unless ( -d "$picard_temp" ) {
        mkdir $picard_temp
          || warn "Could not create Directory $picard_temp $!\n";
    }
    ...
    #exit();
    my $merge_input = "";
    opendir( DIR, $temp_output_folder )
      || die("Cannot open directory $temp_output_folder");
    my @files = readdir(DIR);
    closedir(DIR);
    ...
    if ( scalar(@$files_to_merge) > $merge_limit ) {
        ...
        my @final_temp_bam = ();
        my $temp_dir =
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$mapper_name";
        unless ( -d $temp_dir ) {
            mkdir($temp_dir);
        }
        ...
        for ( my $i = 1 ; $i <= $temp_merge_file_count ; $i++ ) {
            ...
            if ( $merge_input ne "" ) {
                ...
                if ( $i >= 2 ) {
                    my $mergebam_command =
"java -Xmx8g -Dsnappy.disable=true -jar $picard_mergesamfiles_jar $merge_input O=$temp_dir/$temp_bam SO=coordinate VALIDATION_STRINGENCY=SILENT MAX_RECORDS_IN_RAM=5000000 TMP_DIR=$picard_temp";
                    system($mergebam_command) == 0
                      || die "Error in Merging mapping files $mergebam_command";
                }
            }
        ...
        if ( $merge_input ne "" ) {
            ### Merging all BAM files into one
            my $mergebam_command =
"java -Xmx8g -Dsnappy.disable=true -jar $picard_mergesamfiles_jar $merge_input O=$output_dir/$output_file SO=coordinate VALIDATION_STRINGENCY=SILENT MAX_RECORDS_IN_RAM=5000000 TMP_DIR=$picard_temp";
            system($mergebam_command) == 0
              || die "Error in Merging mapping files $mergebam_command";
        }
...
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_output_folder'}";
    ...
    my $picard_temp =
      "$args_dict->{'output_directory'}/$args_dict->{'random_id'}/picard_tmp";
    unless ( -d "$picard_temp" ) {
        mkdir $picard_temp
          || warn "Could not create Directory $picard_temp $!\n";
    }
...
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$mapper_name";
        ...
        for ( my $i = 1 ; $i <= $temp_merge_file_count ; $i++ ) {
            ...
            if ( $merge_input ne "" ) {
                ### Merging all BAM files into one
                my $mergebam_command =
"java -Xmx8g -Dsnappy.disable=true -jar $picard_mergesamfiles_jar $merge_input O=$temp_dir/$temp_bam SO=coordinate VALIDATION_STRINGENCY=SILENT TMP_DIR=$picard_temp";
                system($mergebam_command) == 0
                  || die "Error in Merging mapping files $mergebam_command";
            }
        ...
        if ( $merge_input ne "" ) {
            ### Merging all BAM files into one
            my $mergebam_command =
"java -Xmx8g -Dsnappy.disable=true -jar $picard_mergesamfiles_jar $merge_input O=$output_dir/$output_file SO=coordinate VALIDATION_STRINGENCY=SILENT TMP_DIR=$picard_temp";
            system($mergebam_command) == 0
              || die "Error in Merging mapping files $mergebam_command";
        }
...
sub check_mapping_files {
    ...
    foreach my $file (@files) {
        ...
        if ( $file eq "_logs" ) {
            ...
            opendir( DIR, $history_dir )
              || die("Cannot open directory $history_dir");
            ...
            if ( -e $job_xml ) {
                open my $fh, "<$job_xml"
                  or warn "Could not open $job_xml for write $!";
                while ( my $line = <$fh> ) {
                    chomp $line;
...
"\nERROR: $missing_blocks blocks are missing\nplease run same command only with --only-hdfs-download flag\n\n";
        ...
        open my $fh, "<$temp_output_folder/MD5"
          or die "Could not open $temp_output_folder/MD5 for write $!";
...
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_output_folder'}";
    ...
    open( my $ofh, ">" . "$output_dir/$output_file" )
    ...
    foreach my $file (@files) {
            ...
            || $file eq ".DS_Store" )
        {
            ...
            if ( $file_size > 1 ) {
                ...
                if ( !exists $files_to_discard->{$file} ) {
                    if ( $file =~ /exon$/ ) {
                        open( my $fh, "<" . "$temp_output_folder/$file" )
                          or die "could not open $_";
                        while ( my $line = <$fh> ) {
                            print $ofh "$line";
                        }
