...
#novoalign -d ../reference/2R-2Mbp.nix -f $f1 $f2 -i 350,50 -F STDFQ -o SAM > $o
    my $cmd1 =
      "$mapper_path -d $ref -f $read1_fastq $read2_fastq $mapper_args -o SAM";
    print STDERR "cmd1: $cmd1\n";
    Utility::runCommand( $cmd1, "novoalign of $read1_fastq and $read2_fastq" )
      == 0 || die "Error novoalign of $read1_fastq and $read2_fastq";

}
else {
    my $cmd1 = "$mapper_path -d $ref -f $read1_fastq $mapper_args -o SAM";
    print STDERR "cmd1: $cmd1\n";
    Utility::runCommand( $cmd1, "novoalign of $read1_fastq" ) == 0
      || die "Error novoalign aln of $read1_fastq";
