...
if ( $read_type1 =~ /(pe|pair)/i ) {
    ...
    if ($mem_mapping) {
        my $cmd1 =
          "$mapper_path mem $mapper_args -M $ref $read1_fastq $read2_fastq";
        print STDERR "cmd1: $cmd1\n";
        Utility::runCommand( $cmd1, "bwa mem of $read1_fastq and $read2_fastq" )
          == 0 || die "Error bwa mem of $read1_fastq and $read2_fastq";
    }
    elsif ($bwasw_mapping) {
        my $cmd1 =
          "$mapper_path bwasw $mapper_args $ref $read1_fastq $read2_fastq";
        print STDERR "cmd1: $cmd1\n";
        Utility::runCommand( $cmd1, "bwasw of $read1_fastq and $read2_fastq" )
          == 0 || die "Error bwasw of $read1_fastq and $read2_fastq";
    }
    else {
        my $cmd1 =
...
#my $cmd3 = "$mapper_path sampe $bwa_sampe_options $ref $read1_sai_file $read2_sai_file $read1_fastq $read2_fastq  > $sam_output";
        my $cmd3 =
"$mapper_path sampe $bwa_sampe_options $ref $read1_sai_file $read2_sai_file $read1_fastq $read2_fastq";
        print STDERR "cmd1: $cmd1\n";
    ...
    if ($mem_mapping) {
        ...
        print STDERR "cmd1: $cmd1\n";
        Utility::runCommand( $cmd1, "bwa mem of $read1_fastq" ) == 0
          || die "Error bwa aln of $read1_fastq";
    }
    elsif ($bwasw_mapping) {
        my $cmd1 = "$mapper_path bwasw $mapper_args $ref $read1_fastq";
        print STDERR "cmd1: $cmd1\n";
        Utility::runCommand( $cmd1, "bwa bwasw of $read1_fastq" ) == 0
          || die "Error bwasw of $read1_fastq";
    }
    else {
        my $cmd1 =
          "$mapper_path aln $mapper_args $ref $read1_fastq > $read1_sai_file";
        my $cmd3 =
"$mapper_path samse $bwa_sampe_options $ref $read1_sai_file $read1_fastq";
