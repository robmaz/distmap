...
my $cmd1 =
  "$mapper_path -q $read1_fastq -t $ref $mapper_args > $exonerate_output";
Utility::runCommand( $cmd1, "exonerate $read1_fastq" ) == 0
  || die "Error exonerate of $read1_fastq; $cmd1";
Utility::runCommand( "$hdfs dfs -put $exonerate_output $output_dir >&2",
    "hdfs dfs -put" ) == 0
