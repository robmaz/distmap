...
sub paired_end_data {
    ...
    $args_dict->{"read_output_folder"} =
      "$args_dict->{'read_folder'}" . "_mapping";
    ...
    if ( scalar(@$files_to_merge) > $merge_limit ) {
        ...
        for ( my $i = 1 ; $i <= $temp_merge_file_count ; $i++ ) {
            ...
            if ( $merge_input ne "" ) {
                ...
                my $mergebam_command =
"java -Xmx4g -Dsnappy.disable=true -jar $picard_mergesamfiles_jar $merge_input O=$temp_dir/$temp_bam SO=coordinate VALIDATION_STRINGENCY=SILENT";
        ...
        if ( $merge_input ne "" ) {
            ### Merging all BAM files into one
            my $mergebam_command =
"java -Xmx4g -Dsnappy.disable=true -jar $picard_mergesamfiles_jar $merge_input O=$output_dir/$output_file SO=coordinate VALIDATION_STRINGENCY=SILENT";
            system($mergebam_command) == 0
              || die "Error in Merging mapping files $mergebam_command";
        }
