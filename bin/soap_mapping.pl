...
if ( $read_type1 =~ /(pe|pair)/i ) {
    my $cmd1 =
"$mapper_path -D $ref_dir/$ref_fasta -a $read1_fastq -b $read2_fastq -o $soap_output -2 $unpaired_hits $mapper_args";
    Utility::runCommand( $cmd1,
        "soap mapping of $read1_fastq and $read2_fastq" ) == 0
      || die "Error soap mapping of $read1_fastq and $read2_fastq";

    my $cmd2 = "$soap2sam_path -p $soap_output > $sam_output";
    Utility::runCommand( $cmd2, "converting $soap_output to $sam_output" ) == 0
      || die "Error converting $soap_output to $sam_output";
}
else {
    my $cmd1 =
"$mapper_path -D $ref_dir/$ref_fasta -a $read1_fastq -o $soap_output -2 $unpaired_hits $mapper_args";
    Utility::runCommand( $cmd1, "soap mapping of $read1_fastq" ) == 0
      || die "Error soap mapping of $read1_fastq and $read2_fastq";
...
if ( $output_format =~ /bam/i ) {
    my $cmd2 =
"$hadoop jar $sartsam_jar I=$sam_output O=$bam_output SO=coordinate VALIDATION_STRINGENCY=SILENT";
