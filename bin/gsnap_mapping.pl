...
if ( $read_type1 =~ /(pe|pair)/i ) {
    if ($split_output) {
        my $cmd1 =
"$mapper_path -d $ref_fasta -D $ref_dir $read1_fastq $read2_fastq $mapper_args --split-output=$counter -A sam";
        Utility::runCommand( $cmd1,
            "gsnap mapping of $read1_fastq and $read2_fastq" ) == 0
          || die "Error gsnap mapping of $read1_fastq and $read2_fastq";
        $sam_output = $counter . ".concordant_uniq";
    }
    else {
        my $cmd1 =
"$mapper_path -d $ref_fasta -D $ref_dir $read1_fastq $read2_fastq $mapper_args -A sam > $sam_output";
        Utility::runCommand( $cmd1,
            "gsnap mapping of $read1_fastq and $read2_fastq" ) == 0
          || die "Error gsnap mapping of $read1_fastq and $read2_fastq";
    }
    ...
    if ($split_output) {
        my $cmd1 =
"$mapper_path -d $ref_fasta -D $ref_dir $read1_fastq $mapper_args --split-output=$counter -A sam";
        Utility::runCommand( $cmd1,
            "gsnap mapping of $read1_fastq and $read2_fastq" ) == 0
          || die "Error gsnap mapping of $read1_fastq and $read2_fastq";
    ...
    {
        my $cmd1 =
"$mapper_path -d $ref_fasta -D $ref_dir $read1_fastq $mapper_args -A sam > $sam_output";
        Utility::runCommand( $cmd1, "gsnap mapping of $read1_fastq" ) == 0
          || die "Error gsnap mapping of $read1_fastq and $read2_fastq";
    }
}
...
"java -Xmx5g -Dsnappy.disable=true -jar $sartsam_jar I=$sam_output O=$bam_output SO=coordinate VALIDATION_STRINGENCY=SILENT";
    Utility::runCommand( $cmd2, "converting SAM into BAM" ) == 0
      || die "Error in converting SAM into BAM";
    Utility::runCommand( "$hdfs dfs -put $bam_output $output_dir >&2",
        "hdfs dfs -put" ) == 0
