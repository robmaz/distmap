...
"=======================================================================\n\n";
        ...
        Utility::createdir(
            "$args_dict->{'output_directory'}/$args_dict->{'ref_dir'}");

        $self->create_genome_index($args_dict);

        my $cmd1 =
          "tar -cvzf $args_dict->{'ref_arch'} $args_dict->{'ref_dir'}/";
        ...
        my $end_time = time();
        my $execution_time =
          Utility::get_execution_time( $start_time, $end_time );
    ...
    unless ( $args_dict->{'mappers_exe_archive'} ne ""
...
sub create_genome_index {
    ...
    foreach my $mapper ( @{ $args_dict->{"mapper_path"} } ) {
        ...
        if ( $mapper =~ m/bwa/i ) {
            $self->bwa_index( $args_dict->{"reference_fasta"},
                $mapper_path, $ref_dir );
        }
        elsif ( $mapper =~ m/(gsnap|gmap)/i ) {
            $reference_file = "reference";
            $self->gsnap_index( $args_dict->{"reference_fasta"},
                $mapper_path, $ref_dir );
        }
        elsif ( $mapper =~ /(tophat|tophat2)/i ) {
            $reference_file = "reference";
            $self->tophat_index( $args_dict->{"reference_fasta"},
                $mapper_path, $ref_dir, $args_dict );
        }
        elsif ( $mapper =~ /(bowtie|bowtie2)/i ) {
            $reference_file = "reference";
            $self->bowtie_index( $args_dict->{"reference_fasta"},
                $mapper_path, $ref_dir, $args_dict );
        }
        elsif ( $mapper =~ /soap/i ) {
            ...
            $self->bsmap_index( $args_dict->{"reference_fasta"},
                $mapper_path, $ref_dir );
        }
        elsif ( $mapper =~ /novoalign/i ) {
            $self->novoalign_index( $args_dict->{"reference_fasta"},
                $mapper_path, $ref_dir );
        }
        elsif ( $mapper =~ /ngm/i ) {
            $self->ngm_index( $args_dict->{"reference_fasta"},
                $mapper_path, $ref_dir );
        }
        else {
            print STDERR
...
sub create_archive {
    ...
    chdir("$args_dict->{'output_directory'}/")
      || die
...
sub gsnap_index {
    ...
    if (
        system("$gmap_build -D $output_dir -d $reference_file $fasta_file") !=
        0 )
    {
        print
"\n\tERROR: mapper executable seems to be not compatible \"$mapper\"; give compatible version of mapper \n\n";
...
sub star_index {
    ...
    my $command =
"$mapper --runMode genomeGenerate --genomeDir $output_dir --genomeFastaFiles $output_dir/$reference_file --runThreadN 4";
...
sub bismark_index {
    ...
    unless ( -e $bowtie_path and -x $bowtie_path ) {
        ...
        $bowtie_path =
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'bin_dir'}/bowtie";

        unless ( -e $bowtie_path and -x $bowtie_path ) {
            $bowtie_path =
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'bin_dir'}/bowtie2";
        }
        unless ( -e $bowtie_path and -x $bowtie_path ) {
            print
"\n\tERROR: bowtie-build or bowtie2-build not found  in the directory \"$path\"\n\n";
        ...
        my ( $name1, $path1, $extension1 ) =
          File::Basename::fileparse( $bowtie_path, '\..*' );
...
"\n\tERROR: \"$bismark_genome_preparation\" executables not found to create genome index \n\n";
    ...
    my $command =
"$bismark_genome_preparation --path_to_bowtie $bowtie_final_path --verbose $output_dir/";
...
sub bowtie_index {
    ...
    unless ( -e $bowtie_build and -x $bowtie_build ) {
        $bowtie_build =
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'bin_dir'}/bowtie-build";

        unless ( -e $bowtie_build and -x $bowtie_build ) {
            $bowtie_build =
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'bin_dir'}/bowtie-build2";
        }
        unless ( -e $bowtie_build and -x $bowtie_build ) {
            print
"\n\tERROR: bowtie-build or bowtie2-build not found  in the directory \"$path\"\n\n";
    ...
    if (
        system(
"$bowtie_build $output_dir/$reference_file $output_dir/$reference_file1"
        ) != 0
      )
    {
        print
...
sub tophat_index {
    ...
    unless (exists $args_dict->{'bowtie_build_version'}
        and -e $args_dict->{'bowtie_build_version'}
        and -x $args_dict->{'bowtie_build_version'}
        and $args_dict->{'bowtie_build_version'} ne "" )
    {
        print
"\n\tERROR: bowtie-build or bowtie2-build not found  in the directory \"$path\"\n\n";
