...
#/usr/local/bin/bowtie2-2.2.6/bowtie2 --phred33 --end-to-end -X 1500 -x ../reference/Dsim_M252_draft_4_all_chr_3bacteria -1 ../fastq/Dsim_base_r11_76bp_1.fq -2 ../fastq/Dsim_base_r11_76bp_2.fq -p 12 -S bam-uf/Dmel-base-r11-76bp.sam
    if ( $read_type1 =~ /(pe|pair)/i ) {
        my $cmd1 =
"$mapper_path $mapper_args -x $ref_dir/$ref_fasta -1 $read1_fastq -2 $read2_fastq";
        Utility::runCommand( $cmd1,
            "bowtie2 mapping of $read1_fastq and $read2_fastq" ) == 0
          || die "Error bowtie mapping of $read1_fastq and $read2_fastq";
    }
    else {
        my $cmd1 =
          "$mapper_path $mapper_args -x $ref_dir/$ref_fasta -U $read1_fastq";
        Utility::runCommand( $cmd1, "bowtie2 mapping of $read1_fastq" ) == 0
          || die "Error bowtie mapping of $read1_fastq and $read2_fastq";
    }
...
#executables/bowtie-1.1.2/bowtie --threads 14 -X 400 -S ref.fasta -1  trimmed_1.fq -2 trimmed_1.fq > mapped.sam
    ...
    if ( $read_type1 =~ /(pe|pair)/i ) {
        my $cmd1 =
"$mapper_path $mapper_args -S $ref_dir/$ref_fasta -1 $read1_fastq -2 $read2_fastq";
        print STDERR "Bowtie: $cmd1\n";
        Utility::runCommand( $cmd1,
            "bowtie mapping of $read1_fastq and $read2_fastq" ) == 0
          || die "Error bowtie mapping of $read1_fastq and $read2_fastq";
    }
    else {
        my $cmd1 =
          "$mapper_path $mapper_args -S $ref_dir/$ref_fasta $read1_fastq";
        Utility::runCommand( $cmd1, "bowtie mapping of $read1_fastq" ) == 0
          || die "Error bowtie mapping of $read1_fastq and $read2_fastq";
    }
