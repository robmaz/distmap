...
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_folder'}"
    ...
    if ( $file_count > 0 or $hdfs_file_count > 0 ) {
        ...
        foreach my $mapper_name ( @{ $args_dict->{"mapper"} } ) {
            ...
            $self->get_mapper_command( $args_dict, $mapper_name, $mapper_path,
                $mapper_args );
...
"$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_folder'}"
    ...
    if ( $file_count > 0 or $hdfs_file_count > 0 ) {
        ...
        foreach my $mapper_name ( @{ $args_dict->{"mapper"} } ) {
            ...
            $args_dict->{"final_output_file"} =
              lc($mapper_name) . "_DistMap_output_Single_end_reads";
            my $shell_script =
                $args_dict->{"output_directory"}
              . "/$args_dict->{'random_id'}/"
              . lc($mapper_name)
...
"$hdfs_exe dfs -chown -R $args_dict->{'username'}:$args_dict->{'groupname'} $output_folder/"
    ...
    if ( system("$hdfs_exe dfs -test -d $output_folder_trim") == 0 ) {
        system(
"$hdfs_exe dfs -chown -R $args_dict->{'username'}:$args_dict->{'groupname'} $output_folder_trim/"
          ) == 0
          || warn
          "Error in changing the ownership of $output_folder_trim folder\n";
    }
    ...
    if ( $mapper_name =~ m/bwa/i ) {
        ...
        if ( $mapper_args ne "" ) {
            ...
            if (    exists $args_dict->{'bwa_sampe_args'}
                and defined( $args_dict->{'bwa_sampe_args'} )
                and $args_dict->{'bwa_sampe_args'} ne "" )
            {
                $mapper_command =
...
"$args_dict->{'mapper_script_name'} --output-dir $output_folder --ref-fasta $args_dict->{'extracted_refarch'}/ref/$args_dict->{'index_name'} --mapper-path $args_dict->{'extracted_execarch'}/bin/bwa --picard-sartsam-jar $args_dict->{'extracted_execarch'}/bin/SortSam.jar --output-format $hadoop_mapping_format --hadoop $args_dict->{hadoop_exe} --hdfs $args_dict->{hdfs_exe} --verbose";
    ...
    if ( $mapper_name =~ m/exonerate/i ) {
        ...
        $mapper_command =
"$args_dict->{'mapper_script_name'} --output-dir $output_folder --ref-fasta $args_dict->{'extracted_refarch'}/ref/$args_dict->{'index_name'} --mapper-path $args_dict->{'extracted_execarch'}/bin/exonerate --mapper-args '$mapper_args' --hadoop $args_dict->{hadoop_exe} --hdfs $args_dict->{hdfs_exe} --verbose";
...
"$args_dict->{'mapper_script_name'} --output-dir $output_folder --ref-fasta $args_dict->{'extracted_refarch'}/ref/$args_dict->{'index_name'} --mapper-path $args_dict->{'extracted_execarch'}/bin/ngm --picard-sartsam-jar $args_dict->{'extracted_execarch'}/bin/SortSam.jar --output-format $hadoop_mapping_format --hadoop $args_dict->{hadoop_exe} --hdfs $args_dict->{hdfs_exe} --verbose";
    ...
    $args_dict->{"mapper_script_path"} =
      "$script_current_directory/bin/$args_dict->{'mapper_script_name'}";
