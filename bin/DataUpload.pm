...
sub start {
	...
	if (system ("$hadoop_exe dfs -test -e $input/$args_dict->{'exec_arch'}")==0) {
		...
		system("$hadoop_exe dfs -D dfs.block.size=$args_dict->{'block_size'} -put $args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'exec_arch'} $input/") == 0 || warn "Error in uploading the $args_dict->{'exec_arch'}\n";
	}
	else {
		system("$hadoop_exe dfs -D dfs.block.size=$args_dict->{'block_size'} -put $args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'exec_arch'} $input/") == 0 || warn "Error in uploading the $args_dict->{'exec_arch'}\n";
	}
...
sub paired_end_data {
	...
	my $file_count=0;
	if (-d "$args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_folder'}") {
	...
	my $hadoop_exe = $args_dict->{'hdfs_exe'};
		...
		print "$hadoop_exe dfs -D dfs.block.size=$args_dict->{'block_size'} -copyFromLocal $args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_folder'}/* $read_folder/\n";   
            system("$hadoop_exe dfs -D dfs.block.size=$args_dict->{'block_size'} -copyFromLocal $args_dict->{'output_directory'}/$args_dict->{'random_id'}/$args_dict->{'read_folder'}/* $read_folder/") == 0 || warn "Error in uploading the $args_dict->{'job_arch'}\n";
