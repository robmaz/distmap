...
if ( $read_type1 =~ /(pe|pair)/i ) {
    my $cmd1 =
      "$mapper_path aln $mapper_args $ref $read1_fastq > $read1_sai_file";
    my $cmd2 =
      "$mapper_path aln $mapper_args $ref $read2_fastq > $read2_sai_file";
    my $cmd3 =
"$mapper_path sampe $bwa_sampe_options $ref $read1_sai_file $read2_sai_file $read1_fastq $read2_fastq  > $sam_output";
    print STDERR "cmd1: $cmd1\n";
    print STDERR "cmd2: $cmd2\n";
    print STDERR "cmd3: $cmd3\n";

    Utility::runCommand( $cmd1, "bwa aln of $read1_fastq" ) == 0
      || die "Error bwa aln of $read1_fastq";
    Utility::runCommand( $cmd2, "bwa aln of $read2_fastq" ) == 0
      || die "Error bwa aln of $read2_fastq";
    Utility::runCommand( $cmd3,
        "bwa sampe of $read1_sai_file and $read2_sai_file" ) == 0
      || die "Error bwa sampe of $read1_sai_file and $read2_sai_file";
}
else {
    my $cmd1 =
      "$mapper_path aln $mapper_args $ref $read1_fastq > $read1_sai_file";
    my $cmd3 =
"$mapper_path samse $bwa_sampe_options $ref $read1_sai_file $read1_fastq  > $sam_output";

    Utility::runCommand( $cmd1, "bwa aln of $read1_fastq" ) == 0
      || die "Error bwa aln of $read1_fastq";
    Utility::runCommand( $cmd3, "bwa sampe of $read1_sai_file" ) == 0
      || die "Error bwa sampe of $read1_sai_file";
...
#my $cmd4 = "$hadoop jar $sartsam_jar I=$sam_output O=$bam_output SO=coordinate VALIDATION_STRINGENCY=SILENT";
    my $cmd4 =
"java -Xmx5g -Dsnappy.disable=true -jar $sartsam_jar I=$sam_output O=$bam_output SO=coordinate VALIDATION_STRINGENCY=SILENT";
    Utility::runCommand( $cmd4, "converting SAM into BAM" ) == 0
      || die "Error in converting SAM into BAM";

#if (Utility::runCommand("$hdfs dfs -test -e $output_dir/$bam_output >&2", "hdfsp dfs -test") == 1) {
#	Utility::runCommand("$hdfs dfs -rm -r $output_dir/$bam_output >&2", "hdf dfs -rm -r") == 0 || die "hdfs dfs -rm -r command failed";
#}
#if (system("$hdfs dfs -test -e $output_dir/$bam_output")==0) {
#	system("$hdfs dfs -rm -r $output_dir/$bam_output") == 0 || warn "Error in deleting the $output_dir/$bam_output\n";
#	Utility::runCommand("$hdfs dfs -put $bam_output $output_dir/ >&2", "hdfsp dfs -put") == 0 || die "hdfs dfs -put command failed";
#}
#else {
#	Utility::runCommand("$hdfs dfs -put $bam_output $output_dir/ >&2", "hdfsp dfs -put") == 0 || die "hdfs dfs -put command failed";
#}
    print STDERR "cmd4: $cmd4\n";

    #if (system("$hdfs dfs -test -e $output_dir/$bam_output")!=0) {
    Utility::runCommand( "$hadoop fs -put $bam_output $output_dir >&2",
        "hadoop fs -put" ) == 0
    ...
    open my $sfh, "<$sam_output"
      or die "Could not open $sam_output for write $!";
